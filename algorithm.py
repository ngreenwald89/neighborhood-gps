"""
File has functions to query neighborhood database based on user form input. 
get_results() is the run function that calls the query function make_queries(), determines the best results, and returns neighborhood data to user
make_queries() queries a database table for each user input, and returns the neighborhoods that best fit that user input.
...
# picture links compiled by Jack and Sulekha
def get_nb_data(nb_list, count):
	...
	# get age_median, income_median, rent_median, and other data to display to user on front end
	data = []
	for nb in nb_list:
		nb_dict = {}
...
def get_results(form_dict):
	# performs each query and gathers neighborhood results
